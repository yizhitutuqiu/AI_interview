...
2. **多模态 AI 情绪助理**：
   - 侧边栏搭载 AI 面试助理，利用 `doubao-seed-2-0-lite` 视觉模型。
   - 每 5 秒自动截取用户的摄像头画面并发送给模型进行情绪分析。
   - 实时判断候选人状态（如紧张、走神等），并在侧边栏给出温和友好的提示与鼓励。
3. **双阶段限时系统**：
...
└── html/                  # 前端静态文件目录
    ...
    └── script.js          # 前端交互逻辑（流式接收、倒计时、摄像头截帧等）
```
